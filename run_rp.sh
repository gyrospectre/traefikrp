...
sudo docker exec -it traefikrp_keycloak_1 /opt/jboss/keycloak/bin/kcadm.sh create users -r rp -s username=testuser -s enabled=true --server http://$LOCAL_IP:8080/auth --realm master --user admin --password admin > /dev/null
sudo docker exec -it traefikrp_keycloak_1 /opt/jboss/keycloak/bin/kcadm.sh set-password -r rp --username testuser --new-password NEWPASSWORD --temporary --server http://$LOCAL_IP:8080/auth --realm master --user admin --password admin > /dev/null
echo " \e[32mdone\e[39m"
